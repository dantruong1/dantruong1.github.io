...
export function HeroBento({
  ...
  return (
    ...
    >
      ...
      <motion.div variants={item} className="lg:col-span-5 flex flex-col justify-between">
          ...
          <div className="grid grid-cols-2 gap-2.5 sm:gap-3.5">
            {content.scrapbook?.map((photo) => (
              <motion.div
                ...
                className={`polaroid-frame p-2 sm:p-2.5 rounded-lg group cursor-pointer relative transition-shadow hover:shadow-lg ${photo.rotate}`}
              >
                ...
                <div className="relative overflow-hidden rounded aspect-[4/3] bg-parchment-dark dark:bg-night-card-alt">
                  ...
                  <span className="absolute bottom-1 left-1 bg-card/90 dark:bg-night-card/90 backdrop-blur-sm text-espresso dark:text-night-text text-[9px] font-mono px-1.5 py-0.5 rounded-full flex items-center gap-1 border border-espresso/5 shadow-xs">
                    ...
                    <span className="truncate max-w-[70px] sm:max-w-[90px]">{photo.badge}</span>
                  </span>
                </div>
        ...
        >
          <a
            ...
            className="bg-mocha-soft/60 dark:bg-night-card-alt hover:bg-mocha-soft/90 dark:hover:bg-night-card rounded-cozy px-4 sm:px-5 py-2.5 flex items-center gap-3 border border-mocha-light/50 dark:border-night-border w-full justify-center transition-all duration-200 hover:scale-[1.02] hover:shadow-sm cursor-pointer group"
          >
            <img
              src="images/cozy/gameboy-vines.png"
              alt="Cozy gameboy doodle"
              className="w-7 h-7 sm:w-8 sm:h-8 object-contain opacity-80 group-hover:scale-110 transition-transform duration-200"
            />
            <span className="font-hand text-sm sm:text-base text-espresso-muted dark:text-night-muted group-hover:text-espresso dark:group-hover:text-night-text transition-colors">
      ...
      <motion.div variants={item} className="lg:col-span-7 flex flex-col gap-5 sm:gap-7">
        {/* Main Bio Card */}
        <Card className="p-5 sm:p-7 md:p-9 bg-card dark:bg-night-card border-espresso/8 dark:border-night-border relative overflow-hidden flex-1 flex flex-col justify-between shadow-cozy">
          <div className="washi-tape washi-tape-top-right" />

          <div>
            <h1 className="font-serif text-3xl sm:text-4xl md:text-5xl font-bold text-espresso dark:text-night-text tracking-tight mb-2 sm:mb-3 leading-[1.15]">
              {content.hero.name}
            </h1>

            <p className="text-xs sm:text-sm font-mono text-matcha-dark dark:text-matcha-glow font-medium mb-1 tracking-wide">
              {content.hero.role}
            </p>
            <p className="text-xs text-espresso-muted dark:text-night-muted font-sans mb-4 sm:mb-6">
              {content.hero.subrole}
            </p>

            {/* Core Narrative Paragraphs with balanced spacing & hierarchy */}
            <div className="space-y-3 sm:space-y-4 text-[14px] sm:text-[15px] text-espresso-light dark:text-night-muted leading-[1.75] sm:leading-[1.8] font-sans">
              <p>
          ...
          <div className="flex flex-wrap gap-2 sm:gap-2.5 pt-5 sm:pt-6 mt-5 sm:mt-6 border-t border-espresso/8 dark:border-night-border">
            <button
              onClick={scrollToCozyCorner}
              className="text-xs font-mono text-terracotta dark:text-terracotta-glow bg-terracotta-soft/70 dark:bg-terracotta/30 hover:bg-terracotta-soft dark:hover:bg-terracotta/40 px-3.5 sm:px-4 py-2 rounded-full flex items-center gap-1.5 transition-all duration-200 hover:shadow-xs hover:-translate-y-0.5 border border-terracotta/20 font-semibold"
            >
            ...
            <button
              ...
              className="text-xs font-mono text-matcha-dark dark:text-matcha-glow bg-matcha-soft dark:bg-matcha-dark/40 hover:bg-matcha-light px-3.5 sm:px-4 py-2 rounded-full transition-all duration-200 hover:-translate-y-0.5"
            >
              About Me
            </button>
            <button
              onClick={() => onNavigate('recommendations')}
              className="text-xs font-mono text-espresso-muted dark:text-night-muted hover:text-espresso dark:hover:text-night-text bg-card-alt dark:bg-night-card-alt hover:bg-parchment-dark px-3.5 sm:px-4 py-2 rounded-full transition-all duration-200 border border-espresso/5 dark:border-night-border hover:-translate-y-0.5"
            >
              Recommendations
            </button>
            <button
